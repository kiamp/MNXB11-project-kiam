...
// constructor
temp_vs_years::temp_vs_years(const std::string &filename)
...
void temp_vs_years::ReadFile() {
  ...
  while (file >> date >> time >> degrees) {   // iterates through file
    ...
    if (year > 2022)
      break;  // 2023 not full, so we only want values until end of 2022
...
void temp_vs_years::GetYearlyAverage() {
  ...
  for (size_t i = 0; i < years.size(); i++) {
    if (years[i] == current_year) {  // sums all temperatures from one year
      ...
      count =
          1;  // count set back to 1 (not 0 because we are already in the loop)
...
void temp_vs_years::CreateHistogram() {
  ...
  TH1D *histogram = new TH1D("histogram",
                             "Average yearly temperature over the years;Year; "
                             "Average Temperature [#circC]",
                             nbins, min_year - 0.5, max_year + 0.5);
  ...
  histogram->Fit(lin_fit,
                 "R");  // R=used specified range of histogram for fitting
