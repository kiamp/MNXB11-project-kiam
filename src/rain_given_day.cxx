...
// constructor
rain_given_day::rain_given_day(const std::string &filename)
    : inputFilename(filename) {}  // this one is used in main

// just going through the file and appending years_1, rain_1, years_2, rain_2 if
// we are in the selected 50 year period and the date is 15.2
void rain_given_day::ReadFile() {
  std::ifstream file(inputFilename);  //! needs to be changed if position of
                                      //! lund_cleaned.txt is moved
  if (!file.is_open()) {
...
void rain_given_day::CreateHistogram() {
  ...
  // first period
  auto *histogram_1 = new TH1D(
      "histogram1", "Rainfall on 15/02 from 1863-1913 ;Rainfall [mm];Entries",
      50, 0, 12);
  histogram_1->SetFillColor(kBlue + 1);  // colour of bars is blue
  histogram_1->SetLineColor(kBlack);     // outline black
  histogram_1->SetFillStyle(3004);       // sets special coloring so we can
                                    // distinguish them when both drawn together
  ...
  auto *histogram_2 = new TH1D(
      "histogram2", "Rainfall on 15/02 from 1972-2022 ;Rainfall [mm];Entries",
      50, 0, 12);
  histogram_2->SetFillColor(kRed + 1);  // colour of bars is blue
  histogram_2->SetLineColor(kBlack);    // outline black
  histogram_2->SetFillStyle(3005);      // sets special coloring so we can
                                    // distinguish them when both drawn together
